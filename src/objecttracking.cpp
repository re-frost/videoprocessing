...
ObjectTracking::ObjectTracking(string filename) : filename_(filename)
{
    int counter = 0;
    ...
    if (filename_ != ""){
        capture.open(filename_, cv::CAP_ANY);
    }else {
        capture.open(0);
    }
    while(true) {
        int wk = cv::waitKey(150);
        capture >> m_mSrcFrame3;
        if (!m_mSrcFrame3.empty()) {
        ObjectTracking::frameprocessing();
        ObjectTracking::Detecttarget();
        ObjectTracking::TrackTarget();

        imshow("Origin Image", m_mSrcFrame3);
        imshow("otsu threshold", m_mThreshold);
        }else if (m_mSrcFrame3.empty()){
            // Autorepeat
...
void ObjectTracking::frameprocessing() {
...
void ObjectTracking::Detecttarget() {
...
void ObjectTracking::TrackTarget() {
